...
if (isset($_GET["returnto"])) {
    ...
    $HTMLOUT .= "<br /><td class='rowhead'>Name</td><td class='col1' align='left'>$s</td></tr>\n";
    $HTMLOUT .= "<tr><td class='rowhead' width='1%'>{$lang['details_download']}</td>
    <td class='col1' width='99%' align='left'><a class='index' href='download.php?torrent=$id'>" . htmlspecialchars($row["filename"]) . "</a></td></tr>";
if($row['box'] == 'yes') {
    $HTMLOUT.= "<tr>
...
if (!empty($row["descr"]))
    $HTMLOUT .= "<tr>
    <td class='col1' style='vertical-align:top'>{$lang['details_description']}</td>
    <td class='col1'><div style='background-color:#444;width:100%;height:150px;overflow: auto'>". str_replace(array("\n", "  "), array("<br />\n", "&nbsp; "), format_comment( $row["descr"] ))."</div></td></tr>";
if (get_user_class() >= UC_POWER_USER && $row["nfosz"] > 0)
    $HTMLOUT .= "<tr>
    <td class='rowhead'>{$lang['details_nfo']}</td>
    <td class='col1' align='left'><a href='viewnfo.php?id=$row[id]'><b>{$lang['details_view_nfo']}</b></a> (" .mksize($row["nfosz"]) . ")</td></tr>\n";
if ($row["visible"] == "no")
