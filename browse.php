...
if (isset($cleansearchstr))
    ...
    $title = '';
    $HTMLOUT .= "<div id='wrapper' style='width:95%;border:1px solid black;background-color:#444;'>";
require_once "include/searchcloud_functions.php";
    $HTMLOUT .= cloud() . "</div>";
    $HTMLOUT .= "<form method='get' action='browse.php'>
    <table class='bottom'>
    <tr>
    <td class='col1' class='bottom'>
    <table class='bottom'>
    <tr>";
    $i = 0;
    $catsperrow = 16;
    ...
    $HTMLOUT .= "<td class='bottom' style='padding-bottom: 2px;padding-left: 7px;align:left;border:0px solid;'>
    <a class='catlink' href='browse.php?cat={$cat['id']}'>
    <img width='42' src=/".$FMED['pic_base_url']."caticons/" . htmlspecialchars($cat['image']) . "></a></td>\n";
    $i++;
}
    $alllink = "<div class='col1' align='left'>(<a href='browse.php?all=1'><b>{$lang['browse_show_all']}</b></a>)</div>";
    $ncats = count($cats);
...
if ($CURUSER['clear_new_tag_manually'] == 'yes') {
    ...
    <table class='main'>
    <tr>
    <td class='col1' class='bottom' style='padding: 1px;padding-left: 10px'>
...
if ($ncats % $catsperrow == 0){
    ...
    <tr>
        ...
        <div align='center'>
          ...
          <a href='?clear_new=1'>
          <input type='submit' value='clear new tag' class='button' /></a>
          <a href='?clear_new=1'></a>
        </div>
        </td>
      </tr>
      </table>
...
if (isset($cleansearchstr)){
    ...
    $HTMLOUT .="<br /><div width='98%' class='headline'>&nbsp;-&nbsp;{$lang['browse_ddls']}</div>
    <div align='center' class='headbody'> <div width='98%' class='headbody'>";
if ($count){
