...
if ($arr["uploaded"] > 0)
    ...
    $htmlout .= "<tr>
    <td class='col1' style='padding: 0px'>$catimage</td>\n
    " ."
    <td class='col1'>
    <a href='details.php?id=$arr[torrent]&amp;hit=1'><b>" . htmlspecialchars($arr["torrentname"]) ."</b></a></td>
    <td class='col1' align='center'>$size</td>
    <td class='col1' align='right'>$seeders</td>
    <td class='col1' align='right'>$leechers</td>
    <td class='col1' align='center'>$uploaded</td>\n
    " ."
    <td class='col1' align='center'>$downloaded</td>
    <td class='col1' align='center'>$ratio</td></tr>\n";
}
    $htmlout .= "</table>\n";
    return $htmlout;
}
...
if (mysqli_num_rows($r) > 0){
    $torrents = "<table align='center' border='0' cellpadding='6' cellspacing='1' width='100%'>\n
    " ."
    <tr>
    ...
    $torrents .= "<tr>
    <td class='col1' style='padding: 0px'>$cat</td>
    <td>
    <a href='details.php?id=" . $a['id'] . "&amp;hit=1'><b>" . htmlspecialchars($a["name"]) . "</b></a></td>
    " ."
    <td class='col1' align='right'>{$a['seeders']}</td>
    <td class='col1' align='right'>{$a['leechers']}</td></tr>\n";
}
    $torrents .= "</table>";
}
...
if ($friend)
    ...
    <td class='col1' align='left' width='99%'>{$joindate}</td></tr>
    <tr>
    <td class='rowhead'>{$lang['userdetails_seen']}</td>
    <td class='col1' align='left'>{$lastseen}</td></tr>";
...
if ($connect == "yes"){
    ...
    <td class='col1' align='left'>".$connectable."</td></tr>";
    $port= $a[1];
    $agent = $a[2];
if (!empty($port))
    $HTMLOUT .= "<tr>
    <td class='rowhead'>{$lang['userdetails_port']}</td>
    <td class='col1' align='left'>".$port."</td>
    </tr>
    <tr>
    <td class='rowhead'>{$lang['userdetails_client']}</td>
    <td class='col1' align='left'>".htmlentities($agent)."</td></tr>";
if ($CURUSER['class'] >= UC_MODERATOR)
    $HTMLOUT .= "<tr>
    <td class='rowhead'>{$lang['userdetails_email']}</td>
    <td class='col1' align='left'><a href='{$FMED['baseurl']}/email-gateway.php?id={$user['id']}'>{$user['email']}</a></td></tr>\n";
if (isset($addr))
    $HTMLOUT .= "<tr>
    <td class='rowhead'>{$lang['userdetails_address']}</td>
    <td class='col1' align='left'>$addr</td></tr>\n";
    $HTMLOUT .= "<tr>
    <td class='rowhead'>{$lang['userdetails_uploaded']}</td>
    <td class='col1' align='left'>".mksize($user["uploaded"])."</td>
    </tr>
    <tr>
    <td class='rowhead'>{$lang['userdetails_downloaded']}</td>
    <td class='col1' align='left'>".mksize($user["downloaded"])."</td></tr>";
...
if ($sr >= 4)
    ...
    <td class='col1' align='left' valign='middle' style='padding-top: 1px; padding-bottom: 0px'>$sr</td></tr>\n";
}
if ($user["avatar"])
    $HTMLOUT .= "<tr>
    ...
    <td class='col1' align='left'>" . htmlspecialchars($user["gender"]) . "</td></tr>\n";
    $HTMLOUT .="<tr>
    <td class='rowhead'>{$lang['userdetails_dobBirthday']}</td>
    <td class='col1' align='left'>" . htmlspecialchars($user["birthday"]) . "</td></tr>\n";
    $HTMLOUT .= "<tr>
    <td class='rowhead'>{$lang['userdetails_class']}</td>
    <td class='col1' align='left'><img src=" . get_user_class_image($user["class"]) . " " . get_user_class_name($user["class"]) . "</td></tr>\n";
    $HTMLOUT .= "<tr>
...
if ($torrentcomments && (($user["class"] >= UC_POWER_USER && $user["id"] == $CURUSER["id"]) || $CURUSER['class'] >= UC_MODERATOR))
    $HTMLOUT .= "<td class='col1' align='left'><a href='userhistory.php?action=viewcomments&amp;id=$id'>$torrentcomments</a></td></tr>\n";
    else
    $HTMLOUT .= "<td class='col1' align='left'>$torrentcomments</td></tr>\n";
...
if ($forumposts && (($user["class"] >= UC_POWER_USER && $user["id"] == $CURUSER["id"]) || $CURUSER['class'] >= UC_MODERATOR))
    $HTMLOUT .= "<td class='col1' align='left'><a href='userhistory.php?action=viewposts&amp;id=$id'>$forumposts</a></td></tr>\n";
    else
    $HTMLOUT .= "<td class='col1' align='left'>$forumposts</td></tr>\n";
if (isset($torrents))
    $HTMLOUT .= "<tr valign=\"top\">
    <td class=\"rowhead\" width=\"10%\">{$lang['userdetails_uploaded_t']}</td>
    <td class=\"col1\" align=\"left\" width=\"90%\">
...
if ($arr["uploaded"] > 0)
    ...
    <td class='col1' align='left' width='99%'>[ <a href=\"userdetails.php?id=$id&amp;snatched_table=1#snatched_table\" class=\"sublink\">Show</a> ]  - $count_snatched <font color='red'><b>staff only!!!</b></font></td></tr>";
}
    $completed = "";
...
if ($user["info"])
    $HTMLOUT .= "<tr valign='top'>
    <td class='col1' align='left' colspan='2' class='text' bgcolor='#F4F4F0'>" . format_comment($user["info"]) . "</td></tr>\n";
...
if ($CURUSER['class'] >= UC_MODERATOR && $user["class"] < $CURUSER['class']){
    ...
    $HTMLOUT .= "<table align='center' border='0' cellpadding='6' cellspacing='1' width='100%'>\n";
    $HTMLOUT .= "<tr><td class='rowhead'>{$lang['userdetails_title']}</td>
    <td class='col1' colspan='2' align='left'>
    <input type='text' size='60' name='title' value='" . htmlspecialchars($user['title']) . "' /></td></tr>\n";
    $avatar = htmlspecialchars($user["avatar"]);
    $HTMLOUT .= "<tr><td class='rowhead'>{$lang['userdetails_avatar_url']}</td>
    <td class='col1' colspan='2' align='left'>
    <input type='text' size='60' name='avatar' value='$avatar' /></td></tr>\n";
...
if ($CURUSER['class'] == UC_MODERATOR && $user["class"] > UC_VIP)
    ...
    <td class='col1' colspan='2' align='left'><select name='class'>\n";
if ($CURUSER['class'] == UC_MODERATOR)
    $maxclass = UC_VIP;
    else
    ...
    $HTMLOUT .= "<tr><td class='rowhead'>{$lang['userdetails_comment']}</td>
    <td class='col1' colspan='2' align='left'>
    <textarea cols='60' rows='6' name='modcomment'>$modcomment</textarea></td></tr>\n";
    $warned = $user["warned"] == "yes";
    $HTMLOUT .= "<tr><td class='rowhead'" . (!$warned ? " rowspan='2'": "") . ">{$lang['userdetails_warned']}</td>
    <td class='col1' align='left' width='20%'>" .
    ( $warned
...
if ($warneduntil == 0)
    $HTMLOUT .= "<td class='col1' align='center'>{$lang['userdetails_dur']}</td></tr>\n";
    else{
    $HTMLOUT .= "<td class='col1' align='center'>{$lang['userdetails_until']} ".get_date($warneduntil, 'DATE');
    $HTMLOUT .= " (" . mkprettytime($warneduntil - time())  . " {$lang['userdetails_togo']})</td></tr>\n";
}
    }else{
    $HTMLOUT .= "<td class='col1'>{$lang['userdetails_warn_for']} <select name='warnlength'>\n";
...
if ($CURUSER['class'] >= UC_STAFF) {
    ...
    <td class='col1' align='left' width='20%'>".($immunity ? "<input name='immunity' value='42' type='radio' />Remove immune Status" : "No immunity Status Set")."</td>\n";
if ($immunity){
if ($user['immunity'] == 1)
    $HTMLOUT .= '<td class="col1" align="center">(Unlimited Duration)</td></tr>';
    else
    $HTMLOUT .= "<td class='col1' align='center'>Until ".get_date($user['immunity'], 'DATE'). " (".mkprettytime($user['immunity'] - time()). " to go)</td></tr>";
    } else{
    $HTMLOUT .= '<td class="col1">Immunity for <select name="immunity">
