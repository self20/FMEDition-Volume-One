...
if($doUpdate){
    ...
    $HTMLOUT .= "<tr>
    <td class='col1'>".get_date($arr['added'],'')."</td>
    <td class='col1' align='left'>{$arr['first']}</td>
    <td class='col1' align='left'>{$arr['last']}</td>
    <td class='col1' align='left'><a href='userdetails.php?id={$arr['addedby']}'>{$arr['username']}</a></td>
    <td class='col1' align='left'>".htmlentities($arr['comment'], ENT_QUOTES)."</td>
    <td class='col1'><a href='admin.php?action=bans&amp;remove={$arr['id']}'>{$lang['text_remove']}</a></td>
    </tr>\n";
}
    $HTMLOUT .= "</table>\n";
}
    ...
    $HTMLOUT .= "<h2>{$lang['text_addban']}</h2>
      ...
      <table border='1' cellspacing='0' cellpadding='5'>
        <tr>
          ...
          <td class='col1'><input type='text' name='first' size='40' /></td>
        </tr>
        <tr>
          <td class='rowhead'>{$lang['table_lastip']}</td>
          <td class='col1'><input type='text' name='last' size='40' /></td>
        </tr>
        <tr>
          <td class='rowhead'>{$lang['table_comment']}</td>
          <td class='col1'><input type='text' name='comment' size='40' /></td>
        </tr>
        <tr>
          <td class='col1' colspan='2' align='center'>
          <input type='submit' name='okay' value='{$lang['btn_add']}' class='btn' /></td>
        </tr>
        <tr>
          <td class='col1' colspan='2' align='center'>
          <input type='submit' name='cacheit' value='{$lang['btn_cache']}' class='btn' /></td>
        </tr>
      </table>
      </form>";
}
